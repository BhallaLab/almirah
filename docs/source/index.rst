...
With some time on config writing, napi can:
...
<https://github.com/girishmm/napi>`_:

.. code-block:: bash

		$ git clone https://github.com/girishmm/napi.git
		$ cd napi
		$ pip install .
...
napi relies heavily on configs to guide its tasks. The
...
Can you use napi to manage your dataset? Of course. Have a look at
:ref:`translate` section for files to tweak to achieve this.

Who uses napi?
--------------

napi was primarily developed to support and maintain the data
generated by the ADBS study. But we believe it can aid with other
datasets too! Even non-neuroimaging ones!

If napi was able to help with your dataset, we would love to hear
about it.
...
napi is made available under the MIT License. For more details, see
`LICENSE <https://github.com/girishmm/napi/blob/dev/LICENSE>`_.
...
We try to make napi robust, but there might be things we missed. If it
...
on the `repository <https://github.com/girishmm/napi>`_ or drop by
`Github Discussions <https://github.com/girishmm/napi/discussions>`_.

Citing
------

If you use napi to help with your dataset, please consider citing this
repository until a journal article is specifically available for napi.
...
napi builds on other libraries and some parts are implemented as a
